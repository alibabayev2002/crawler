...
class DashboardController extends Controller
{
    public function getAdvertises(Request $request)
    {
        $advertises = Advertise::query()
            ...
            ->when($request->get('room_count'), function ($query) use ($request) {
                ...
                $query->having('area_int', '>=', $request->get('area_m_min'));
            })->when($request->get('area_m_max'), function ($query) use ($request) {
                $query->having('area_int', '<=', $request->get('area_m_max'));
            })->when($request->get('area_sot_max'), function ($query) use ($request) {
                $query->having('land_int', '<=', $request->get('area_sot_max'));
            })->when($request->get('area_sot_min'), function ($query) use ($request) {
                $query->having('land_int', '>=', $request->get('area_sot_min'));
            ...
            ->selectRaw('advertises.*,CAST(advertises.price as UNSIGNED) as price_int,CAST(advertises.area as UNSIGNED) as area_int,CAST(advertises.land as UNSIGNED) as land_int')
            ->latest();
    ...
    public function index(Request $request)
    {
        ...
        return Inertia::render('Dashboard', compact('advertises', 'notParsedCount','subways', 'settlements', 'districts', 'categories'));
    }
    ...
    public function favorites(Request $request)
    {
        ...
        $advertises = $this->getAdvertises($request)->whereIn('id', $ids)
            ->paginate(10);
